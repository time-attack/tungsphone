...
build_selected_package() {
  ...
  print ""
  print "==> Starting build: ${BUILD_SCHEME_LABEL} / ${BUILD_TYPE_LABEL}"
  make clean >/dev/null
  build_subprojects
  ...
  if [[ -z "$deb" ]]; then
    print -u2 "No build artifact found in ${ROOT_DIR}/packages."
    exit 1
  fi
